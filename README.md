...
🔗 **GitHub**: https://github.com/YDMY007/astrbot_plugin_jusuo
...
### v1.10 · 2026-07-06
- 撤回时间可配置（WebUI recall_seconds + `设置撤回` 指令）；所有查询结果标题行显示撤回时间
- 设置阈值指令回写 AstrBot 配置，WebUI 同步显示
- 播报状态持久化到文件（`_poll_state.json`），插件重载不再重复播报
...
- 动物提醒指令（经验动物/金币动物/提醒查询/动物提醒/取消动物提醒等）在白名单群内**无需 @ 即可触发**（之前仅 @filter.command 注册，群里不 @ 机器人就无反应）。
- 修复白名单群自动检测失效：指令事件 `message_obj.group_id` 为 None 时，回退 `event.get_group_id()` 取裸 id（去除 `OneBotV11_group_` 等前缀），使自动登记与监听判断对齐。
- 三处异常日志补充类型名（`type(e).__name__`）：source1 REST 请求失败 / asyncio.gather 查询异常 / 牧场预设加载失败，便于定位网络或超时问题。
